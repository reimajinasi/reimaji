...
export default async function Page() {
  const getCachedCourses = unstable_cache(
    async () => fetchQuery(api.courses.list, {}),
    ['courses-list-default'],
  ...
  return (
    <div className='space-y-6'>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'LMS' }]}
      />
      <Section>
            ...
            {courses.length === 0 ? (
              ...
              courses.map((c) => (
                <div key={c._id} className='rounded-[var(--radius)] border border-border p-4'>
                  ...
                  <Link
                    href={`/lms/${c.slug}`}
                    className='mt-3 inline-block rounded bg-primary px-3 py-1 text-primary-foreground'
                  >
                    Lihat Kursus
                  </Link>
                </div>
